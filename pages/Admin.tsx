...
import { 
  ...
  RefreshCw, Bold, Italic, Underline, List, Code, Type, Strikethrough, Eye, Layers, LayoutTemplate, ToggleLeft, ToggleRight
} from 'lucide-react';
...
export const Admin: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'analytics' | 'novels' | 'chapters' | 'users' | 'frontend'>('analytics');
  ...
  const [siteSettings, setSiteSettings] = useState<SiteSettings>({
      showHero: true, showWeeklyFeatured: true, showRankings: true, showRising: true, showTags: true, showPromo: true, enablePayments: true
  });
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
                        ...
                        <div className="space-y-4">
                            {[
                                ...
                                { key: 'showHero', label: 'Weekly Book Hero Section', desc: 'Large banner at the top with the weekly featured novel.' },
                                { key: 'showWeeklyFeatured', label: 'Weekly Featured Grid', desc: 'Grid of 5 featured novels below the hero.' },
                                { key: 'showRankings', label: 'Ranking Lists', desc: 'Three columns for Power, Collection, and Active rankings.' },
                                { key: 'showRising', label: 'Rising Fictions', desc: 'Grid of potential starlet / rising novels.' },
                                { key: 'showTags', label: 'Popular Tags Cloud', desc: 'List of searchable genre tags.' },
                                ...
                                <div key={item.key} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-100 dark:border-slate-700">
                                    ...
                                    <button 
                                        onClick={() => toggleSetting(item.key as keyof SiteSettings)}
                                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${siteSettings[item.key as keyof SiteSettings] ? 'bg-primary' : 'bg-slate-300 dark:bg-slate-600'}`}
                                    >
                                        <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${siteSettings[item.key as keyof SiteSettings] ? 'translate-x-6' : 'translate-x-1'}`} />
                                    </button>
                                </div>
                            ))}
                        </div>
